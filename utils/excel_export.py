...
POLICY_REC_FILL = PatternFill("solid", fgColor="E8D5FF")   # light purple for Policy Recommendation

VERDICT_FILL = {
    "CRITICAL":   PatternFill("solid", fgColor="FFCDD2"),
    "UPGRADE":    PatternFill("solid", fgColor="FFE0B2"),
    "EXTEND":     PatternFill("solid", fgColor="FFF9C4"),
    "REPLACE":    PatternFill("solid", fgColor="BBDEFB"),
    "CLOUD":      PatternFill("solid", fgColor="C8E6C9"),
    "MONITOR":    PatternFill("solid", fgColor="F1F8E9"),
}
...
def _write_sheet(ws, df: pd.DataFrame, title: str,
    ...
    for ri, (_, row) in enumerate(df.iterrows(), 3):
        ...
        for ci, col in enumerate(cols, 1):
            ...
            elif col == "Policy Recommendation":
                c.fill = POLICY_REC_FILL
                c.font = Font(name="Calibri", size=10, bold=True)
            elif col == "Verdict":
                verdict_key = val.upper().split()[0] if val else ""
                c.fill = VERDICT_FILL.get(verdict_key, base_fill)
    ...
    for ci, col in enumerate(cols, 1):
        ...
        if col in (rec_col, "Notes"):
            ...
            ws.column_dimensions[cl].width = 62
        elif col == "Verdict":
...
def export_to_excel(
    ...
    principles: list = None,      # Agent 5 guiding principles
    costs: dict = None,           # Agent 5 vendor cost data
) -> bytes:
    """
    ...
    summary_rows = [
        ...
        ("Agent 2", "Generates expert AI recommendations per row — powered by Claude claude-sonnet-4-6"),
        ...
        ("Agent 5", "Policy Analysis Agent — org policy interview, guiding principles, live cost intelligence, consolidated policy recommendations"),
        ("", ""),
    ...
    # ── Agent 5 Sheet: Guiding Principles ─────────────────────────────────────
    if principles:
        ...
        ws_gp.column_dimensions["D"].width = 50

        # Header
        gp_headers = ["Code", "Title", "Rule", "Trigger"]
        for ci, h in enumerate(gp_headers, 1):
            c = ws_gp.cell(row=1, column=ci, value=h)
            c.font = HDR_FONT; c.fill = HDR_FILL; c.alignment = CENTER; c.border = THIN

        gp_title = ws_gp.cell(row=0+1, column=1)  # reuse header as title via insert
        # Insert title row above headers
        ws_gp.insert_rows(1)
        ws_gp.merge_cells("A1:D1")
        t = ws_gp.cell(row=1, column=1,
                        value=f"Guiding Principles — Generated by Agent 5  |  {ts}")
        t.font = Font(bold=True, size=12, color="FFFFFF", name="Calibri")
        t.fill = PatternFill("solid", fgColor="4C1D95")
        t.alignment = CENTER
        ws_gp.row_dimensions[1].height = 22

        for ri, gp in enumerate(principles, 3):
    ...
    # ── Agent 5 Sheet: Vendor Cost Intelligence ────────────────────────────────
    if costs:
        ws_cost = wb.create_sheet("Vendor Cost Data")
        ws_cost.column_dimensions["A"].width = 30
        ws_cost.column_dimensions["B"].width = 90

        ws_cost.merge_cells("A1:B1")
        tc = ws_cost.cell(row=1, column=1,
                          value=f"Live Vendor Cost Intelligence — Agent 5  |  {ts}")
        tc.font = Font(bold=True, size=12, color="FFFFFF", name="Calibri")
        ...
        for ri, (vendor, summary) in enumerate(costs.items(), 3):
            fill = ALT_FILL if ri % 2 == 0 else WHITE_FILL
    ...
    # ── Agent 5 Sheet: Verdict Summary ────────────────────────────────────────
    if ("Verdict" in os_df.columns and os_df["Verdict"].any()) or \
       ("Verdict" in db_df.columns and db_df["Verdict"].any()):
        ws_v = wb.create_sheet("Policy Verdict Summary")
        ws_v.merge_cells("A1:C1")
        tv = ws_v.cell(row=1, column=1,
                       value=f"Policy Verdict Summary — Agent 5  |  Project: Apr 2026 → Jun 2028  |  {ts}")
        tv.font = Font(bold=True, size=12, color="FFFFFF", name="Calibri")
        tv.fill = PatternFill("solid", fgColor="4C1D95")
        tv.alignment = CENTER
        ws_v.row_dimensions[1].height = 22

        verdicts = ["CRITICAL","UPGRADE NOW","EXTEND + PLAN","REPLACE","CLOUD MIGRATE","MONITOR"]
        row = 3
        for sheet_label, df in [("OS Versions", os_df), ("DB Versions", db_df)]:
            if "Verdict" not in df.columns: continue
            ws_v.cell(row=row, column=1,
                      value=sheet_label).font = Font(bold=True, size=11, color=INFY_BLUE, name="Calibri")
            row += 1
            for verd in verdicts:
                count = df["Verdict"].str.upper().str.startswith(verd.split()[0]).sum() \
                    if "Verdict" in df.columns else 0
                fill_key = verd.split()[0]
                c_label = ws_v.cell(row=row, column=1, value=verd)
                c_count = ws_v.cell(row=row, column=2, value=int(count))
                for c in (c_label, c_count):
                    c.font = Font(name="Calibri", size=10)
                    c.fill = VERDICT_FILL.get(fill_key, WHITE_FILL)
                    c.border = THIN
                row += 1
            row += 1

        ws_v.column_dimensions["A"].width = 22
        ws_v.column_dimensions["B"].width = 10

    # ── Sheets: One OS+DB sheet per historical version (Agent 4) ─────────
    if version_history:
